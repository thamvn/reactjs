    ...
    open = cell => {
        ...
        // asyncCountMines.then(numberOfMines => {
            ...
            if (current.hasMine && this.props.openCells === 0) {
                ...
                alert("Boom!!! First click was on boom !!");
                ...
                if (!cell.hasFlag && !current.isOpen) {
                    ...
                    current.count = mines;
                    this.setState({ rows });
                    if (!current.hasMine && mines === 0) {
                        this.findAroundFlag(cell);
                    }

                    if (current.hasMine && this.props.openCells !== 0) {
                        this.props.endGame();
                        
                        alert("You click on the boom ... Let's try again !!");
                        let newRows = this.createBoard(this.props);
                        this.props.resetGame();
                        this.setState({
                            rows: newRows
